# Colour ontology addendum: epistemic location and authorization

Date: 2026-08-27
Status: active extension; merge into `COLOUR_ONTOLOGY.md` after primary controls close
...
## 2. Recommended `epistemic_location` values

### `source_object`
Colour is a property or reported property of the historical/natural/material object itself.

Question: is the colour part of what evidence must preserve, describe or classify?

### `surrogate_encoding`
Colour information is translated into another carrier: words, letters, hatching, legend, sample, recipe, pigment reproduction.

Question: is the translation decodable, source-grounded and sufficiently precise?

### `fact_proposition`
Colour acts on the factual proposition itself, literally or metaphorically.

Question: does it add unsupported content, rhetorical loading or invention?

Schlözer's 1772 criticism of `Facta färben` belongs here.

### `classification`
Colour is used as a variable for sorting bodies, objects, territories, states or other units.

Question: what observation or convention licenses the category, and how stable is the partition?

### `selection`
Colour assists or marks a decision about which units belong in an analytical set.

Keep separate from classification where possible.
...
Question: who coloured it, when, according to what code/source, and with what copy variance?

## 3. Authorization relation

Add a higher-level analytical relation:

`colour_authorization`
...
This axis prevents a false binary between `objective colour` and `subjective colour`.
...
- carrier: hand-applied pigment;
- operation: mark relation/boundary/change and support overview;
- epistemic locations: `arrangement_system`, `display_inspection`, plus `copy_finishing` at material witness level;
- authorization: `system_assigned` pending exact primary control of explanatory text.

### Gatterer heraldry / Rink hatching

- carrier: monochrome hatch sign;
...
- carrier: engraved monochrome key plus bilingual terms;
- operation: decode/standardise/compress/navigate;
- epistemic location: `surrogate_encoding` and `display_inspection`;
- authorization: `convention_warranted` at minimum; stronger source genealogy requires separate evidence.
...
Finding the same word `Farbe` or the same physical pigment in multiple works does not establish one transferable colour method.

A cross-domain link requires at least one of:
- actor-level cross-reference;
- shared explicit methodological vocabulary;
- common coding rule;
- documented transfer of plates/tables/legends/practices;
